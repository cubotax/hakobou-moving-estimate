...
function buildInviteFlexMessage(estimate) {
    ...
    if (estimate.base_fee > 0) feeBreakdownItems.push({ label: '基本料金', value: estimate.base_fee });
    if (estimate.plan_fee > 0) feeBreakdownItems.push({ label: 'お任せプラン', value: estimate.plan_fee });
    if (estimate.packing_fee > 0) feeBreakdownItems.push({ label: '梱包サービス', value: estimate.packing_fee });
    if (estimate.time_slot_fee > 0) feeBreakdownItems.push({ label: '時間指定', value: estimate.time_slot_fee });
    if (estimate.weekend_holiday_fee > 0) feeBreakdownItems.push({ label: '土日祝加算', value: estimate.weekend_holiday_fee });
    if (estimate.floor_pickup_fee > 0) feeBreakdownItems.push({ label: '集荷先階数料金', value: estimate.floor_pickup_fee });
    if (estimate.floor_delivery_fee > 0) feeBreakdownItems.push({ label: '届け先階数料金', value: estimate.floor_delivery_fee });
    if (estimate.storage_fee > 0) feeBreakdownItems.push({ label: '積み置き料金', value: estimate.storage_fee });
    ...
    const feeBreakdownContents = feeBreakdownItems.length > 0 ? [
        ...
        ...feeBreakdownItems.map(item => ({
            ...
            contents: [
                ...
                { type: 'text', text: `¥${item.value.toLocaleString()}`, size: 'xs', color: '#333333', align: 'end', flex: 1 },
            ],
        })),
    ] : [];
    ...
    try {
        ...
        const { data, error } = await supabase
            ...
            .insert({
                ...
                base_fee: proposalData.baseFee || 0,
                plan_fee: proposalData.planFee || 0,
                packing_fee: proposalData.packingFee || 0,
                time_slot_fee: proposalData.timeSlotFee || 0,
                weekend_holiday_fee: proposalData.weekendHolidayFee || 0,
                floor_pickup_fee: proposalData.floorPickupFee || 0,
                floor_delivery_fee: proposalData.floorDeliveryFee || 0,
                storage_fee: proposalData.storageFee || 0,
                busy_season_fee: proposalData.busySeasonFee || 0,
                distance_fee: proposalData.distanceFee || 0,
                // 住所情報
                pickup_prefecture: proposalData.pickupPrefecture || '',
                pickup_city: proposalData.pickupCity || '',
                pickup_town: proposalData.pickupTown || '',
                pickup_address_detail: proposalData.pickupAddressDetail || '',
                pickup_building: proposalData.pickupBuilding || '',
                delivery_prefecture: proposalData.deliveryPrefecture || '',
                delivery_city: proposalData.deliveryCity || '',
                delivery_town: proposalData.deliveryTown || '',
                delivery_address_detail: proposalData.deliveryAddressDetail || '',
                delivery_building: proposalData.deliveryBuilding || '',
                // 顧客情報
                last_name: proposalData.lastName || '',
                first_name: proposalData.firstName || '',
                last_name_kana: proposalData.lastNameKana || '',
                first_name_kana: proposalData.firstNameKana || '',
