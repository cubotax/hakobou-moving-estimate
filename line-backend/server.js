...
// 署名検証関数（テストのため常に true を返すように修正）
function verifySignature(body, signature) {
  console.log("--- TEST MODE: Signature verification always returns TRUE ---");
  return true;
}
...
app.post("/api/link", (req, res) => {
  try {
    ...
    if (!estimateId || !lineUserId) {
      return res
        .status(400)
        .json({ error: "estimateId and lineUserId are required" });
    }
...
app.post(
  ...
  async (req, res) => {
    console.log(">>> Webhook request received at /webhook");
    ...
    // 署名検証をスキップ
    console.log("Signature check bypassed for testing.");
    ...
    if (events.length === 0) {
      console.log("Webhook verification request received (empty events list)");
      return;
    }
